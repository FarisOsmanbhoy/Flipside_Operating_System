...
} from "@/app/(app)/(operational)/suppliers/actions";
import type { SupplierContact } from "@/lib/database.types";
...
export function SupplierContactsSection({
  ...
  const remove = (c: SupplierContact) =>
    startDelete(async () => {
      if (!confirm(`Delete ${c.name}?`)) return;
      try {
        await deleteSupplierContact(c.id, supplierId);
      } catch (e) {
        push({
          tone: "error",
...
function ContactForm({
  ...
  const [state, action, pending] = useActionState<SupplierContactState, FormData>(
    async (prev, fd) => {
      const result = await saveSupplierContact(prev, fd);
